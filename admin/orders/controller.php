...
function doInsert(){
	...
	function doEdit(){
		...
		if ($_GET['actions']=='confirm') {
			...
			// `tblorder` has no `STATS` column; summary holds order status.
			// Skip updating `tblorder` to avoid SQL errors (was causing empty SET clause).
  	...
  	// 		$res = $customer->single_customer($_GET['customerid']); 
			...
			$messageText = "FROM Bachelor of Science and Entrepreneurs : Your order has been " . $status . ". The amount is " . $cur->PAYMENT;
			$sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '" . $cur->PHONE . "', 'Janno', '" . addslashes($messageText) . "')";
			$mydb->setQuery($sql);
			$mydb->executeQuery();
			...
			foreach ($cur as $result) {
		   	...
		   	$messageText = "FROM Bachelor of Science and Entrepreneurs : Your product has been ordered. The amount is " . $result->PAYMENT;
			$sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '" . $result->OWNERPHONE . "', 'Janno', '" . addslashes($messageText) . "')";
			$mydb->setQuery($sql);
			$mydb->executeQuery();
