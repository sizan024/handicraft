...
function doInsert(){
	...
	if(isset($_POST['submit'])){
						...
						$customer->MNAME 			= isset($_POST['MNAME']) ? $_POST['MNAME'] : '';
						...
						$customer->CITYADD  		= isset($_POST['CITYADD']) ? $_POST['CITYADD'] : ''; 
						$customer->PROVINCE 		= isset($_POST['PROVINCE']) ? $_POST['PROVINCE'] : '';
						$customer->COUNTRY 			= isset($_POST['COUNTRY']) ? $_POST['COUNTRY'] : '';
						$customer->DBIRTH 			= isset($_POST['DBIRTH']) && $_POST['DBIRTH'] != '' ? $_POST['DBIRTH'] : NULL;
						$customer->GENDER 			= $_POST['GENDER'];
					 $customer->PHONE 			= $_POST['PHONE']; 
						$customer->EMAILADD 		= isset($_POST['EMAILADD']) ? $_POST['EMAILADD'] : $_POST['CUSUNAME'];
						$customer->ZIPCODE 			= isset($_POST['ZIPCODE']) ? $_POST['ZIPCODE'] : 0;
	...
	function doDelete(){
		...
		//	$_SESSION['ORDEREDNUM'] = $_POST['ORDEREDNUM'];
			...
			// generate a unique ORDEREDNUM using autonumber
			$autonumber = New Autonumber();
			$res = $autonumber->set_autonumber('ordernumber');
			$ordernum = isset($res->AUTO) ? $res->AUTO : time();
			...
			// assign USERID for order (use logged-in user id if available, otherwise 0)
			$order->USERID			= isset($_SESSION['USERID']) ? $_SESSION['USERID'] : 0;
			$order->create(); 
			...
			// summary will be created once after all order rows are inserted
		  }
		...
		$summary->CLAIMEDADTE		= isset($_POST['CLAIMEDDATE']) && $_POST['CLAIMEDDATE'] != '' ? $_POST['CLAIMEDDATE'] : date("Y-m-d H:i:s");
		$summary->ORDEREDREMARKS 	= 'Your order is on process.';
		$summary->HVIEW 			= 0;
		$summary->USERID		= isset($_SESSION['USERID']) ? $_SESSION['USERID'] : 0;
		$summary->create();
